...
uint8_t Endstops::read() {
    ...
    // If endstop 1 is triggered state = 1
    else if(digitalRead(pin1) == LOW){
        state = 1;
    }
    // If endstop 2 is triggered state = 2
    else if(digitalRead(pin2) == LOW){
        state = 2;
    }
    // If no endstop is triggered state = 0
    else{
        state = 0;
    }
...
bool Endstops::changed() {
    ...
    // If the state of the Endstops has not changed, return false
    return false;
}
