...
//*Endstop-Settings:
const uint8_t ES_TRIGGER = 0; //Endstop Mode (0 = Pullup, 1 = Pulldown) - default: 0
...
const uint8_t GO_TRIGGER = 0; //Go-Button Trigger (0 = Pullup, 1 = Pulldown) - default: 0

//*DC-Motor-Settings:

const uint8_t DC_MAXSPEED = 255;    //DC-Motor Max Speed for both motors (PWM-Value)
...
namespace HR{                   //*DC-Motor Hammerwheel
    ...
    const short SPEED = (reversed) ? -speed : speed;    //Changing direction according to reversed
    const short RS_SPEED = (reversed) ? rs_speed : -rs_speed;    //Changing direction according to reversed
}
...
const uint8_t STP_MODE = 16;                                //Stepper Mode (1 = Full Step, 2 = Half Step, 4 = Quarter Step, 8 = Eighth Step, ...)
const short STP_STEPS = 200;                                //Stepper Steps per Revolution
const float STP_i = 1.2;                                    //Stepper to Schild Gear Ratio
const short STP_POS = short(STP_STEPS/3*STP_i)*STP_MODE;    //Steps per Position change
