...
/// Different drivers might work on another way and therefore need different functions
///	For extending, just create a new class with a fitting name and add needed functions and attributes
///
/*/
	
	enum class Status : bool {Standby, Active}; //Enum for Driver-State if a Standby-Mode is available

/**
 * @class MP6550
 * @brief This class is for controlling the MP6550 Motor Driver.
 *
 * It's a simplified class missing the VISEN pin thus not being able to read the current draw of the motor.
 * If you don't provide a SLP pin, you have to pull the SLP pin to HIGH on another way. Otherwise the driver will stay disabled.
 * If a positive speed value should result in a certain direction, it might be helpful to switch IN1 and IN2 while object creation.
 */
