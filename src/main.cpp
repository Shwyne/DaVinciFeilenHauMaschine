...
{
  NO_ERROR,        //No Error
  HW_TIMEOUT,   //Timeout Hammerwheel: Occurs if the Hall cant detect the magnets for engaging the hammerstop. Hardware to check: Hall-Sensor, DC-Motor, DC-Driver, Arduino, Connections
  SG_TIMEOUT,   //Timeout Sign: Occurs if the Hall cant detect the magnets for nulling the sign-stepper. Hardware to check: Hall-Sensor, Stepper-Driver, Stepper, Arduino, Connections
  WG_TIMEOUT,   //Timeout Weight: Occurs if one of the Endstops should've been triggered but is not. Hardware to check: Endstops, DC-Motor, DC-Driver, Arduino, Connections
  SL_TIMEOUT,   //Timeout Slider: Occurs if one of the Endstops should've been triggered but is not. Hardware to check: Endstops, DC-Motor, DC-Driver, Arduino, Connections
  STPDRV_NOT_CONNECTED, //Stepper-Driver not connected: The DRV8825 is pulling the ?-Pin to high. If the driver isnt present the arduino reads low. Hardware to check: Stepper-Driver, Arduino, Connections
  WG_OVERCURRENT,       //Overcurrent Weight-Motor: Occurs if the current of the DC-Motor is too high. (Only implemented, if a VISEN-Pin is given): Hardware to check: DC-Motor, DC-Driver
  SL_OVERCURRENT,       //Overcurrent Slider-Motor: Occurs if the current of the DC-Motor is too high. (Only implemented, if a VISEN-Pin is given): Hardware to check: DC-Motor, DC-Driver
  COUP_NOT_ATTACHED,    //Servo for coupling not attached: Occurs if the servo is not attached. Hardware to check: Servo, Arduino, Connections
  COUP_NOT_IN_POS,      //Servo for coupling not in expected position: Occurs if the servo is not in the position it should be. Hardware to check: Servo, Arduino, Connections 
  HS_NOT_ATTACHED,      //Servo for hammerstop not attached: Occurs if the servo is not attached. Hardware to check: Servo, Arduino, Connections
  HS_NOT_IN_POS,         //Servo for hammerstop not in expected position: Occurs if the servo is not in the position it should be. Hardware to check: Servo, Arduino, Connections
  UNDEFINED             //Undefined Error: Occurs if an error is thrown but not defined in the enum class.
};

ErrCode erCode = ErrCode::NO_ERROR;   //Error Code for Error Handling. Default: NO_ERROR
void printError(ErrCode erCode);    //Prints the error code to the serial monitor with a short description.
...
enum MachineState : uint8_t {IDLE, RUN, RESET, ERROR};  //States for state machine
...
MachineState initState();   //Initializes the system. Checks if everything is in position and throws an error if not.
...
namespace states{
  ...
  void run();         //Run state: Running motors till interval is or endstops are reached
...
void dsetup(){
  ...
  if(HSsv.attached() == false) erCode = ErrCode::HS_NOT_ATTACHED;
  else Serial.println("Servo Hammerstop attached.");
  COsv.attach();                          //Initializes Servo Kupplung
  if(COsv.attached() == false) erCode = ErrCode::COUP_NOT_ATTACHED;
  else Serial.println("Servo Coupling attached.");
...
void dloop(){   //!For prototyping - delete if not needed anymore and move code to loop()
  ...
  switch(currentState){
    case IDLE:
      states::idle();
      if(digitalRead(pin::GO_BUT) == GO_TRIGGER) currentState = RUN;
      break;
    case RUN:
      states::run();
      if(SLes.read() == Slider::LEFT || WGes.read() == Weight::BOTTOM) currentState = RESET;
      else if(ReachedTimer()) currentState = IDLE; 
      break;
    case RESET:
      states::reset();
      break;
...
void printError(ErrCode erCode){
  switch(erCode){
    ...
    case ErrCode::HW_TIMEOUT:
      Serial.println("TIMEOUT - Hammerwheel: Occurs if the Hall cant detect the magnets for engaging the hammerstop. Hardware to check: Hall-Sensor, DC-Motor, DC-Driver, Arduino, Connections");
      break;
    case ErrCode::SG_TIMEOUT:
      Serial.println("TIMEOUT - Sign: Occurs if the Hall cant detect the magnets for nulling the sign-stepper. Hardware to check: Hall-Sensor, Stepper-Driver, Stepper, Arduino, Connections");
      break;
    case ErrCode::WG_TIMEOUT:
      Serial.println("TIMEOUT - Weight: Occurs if one of the Endstops should've been triggered but is not. Hardware to check: Endstops, DC-Motor, DC-Driver, Arduino, Connections");
      break;
    case ErrCode::SL_TIMEOUT:
      Serial.println("TIMEOUT - Slider: Occurs if one of the Endstops should've been triggered but is not. Hardware to check: Endstops, DC-Motor, DC-Driver, Arduino, Connections");
      break;
    case ErrCode::STPDRV_NOT_CONNECTED:
      Serial.println("NOT CONNECTED - Stepper-Driver: The DRV8825 is pulling the ?-Pin to high. If the driver isnt present the arduino reads low. Hardware to check: Stepper-Driver, Arduino, Connections");
      break;
    case ErrCode::WG_OVERCURRENT:
      Serial.println("OVERCURRENT - Weight-Motor: Occurs if the current of the DC-Motor is too high. (Only implemented, if a VISEN-Pin is given): Hardware to check: DC-Motor, DC-Driver");
      break;
    case ErrCode::SL_OVERCURRENT:
      Serial.println("OVERCURRENT - Slider-Motor: Occurs if the current of the DC-Motor is too high. (Only implemented, if a VISEN-Pin is given): Hardware to check: DC-Motor, DC-Driver");
      break;
    case ErrCode::COUP_NOT_ATTACHED:
      Serial.println("NOT ATTACHED - Servo for coupling : Occurs if the servo is not attached. Hardware to check: Servo, Arduino, Connections");
      break;
    case ErrCode::COUP_NOT_IN_POS:
      Serial.println("NOT IN POS - Servo for coupling: not in expected position - Occurs if the servo is not in the position it should be. Hardware to check: Servo, Arduino, Connections");
      break;
    case ErrCode::HS_NOT_ATTACHED:
      Serial.println("NOT ATTACHED - Servo for hammerstop: Occurs if the servo is not attached. Hardware to check: Servo, Arduino, Connections");
      break;
    case ErrCode::HS_NOT_IN_POS:
      Serial.println("NOT IN POS - Servo for hammerstop: not in expected position - Occurs if the servo is not in the position it should be. Hardware to check: Servo, Arduino, Connections");
      break;
